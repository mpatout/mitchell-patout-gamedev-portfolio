...
Pulse Grid is a 4×4 reaction arcade game built with **Defold 1.9+** and Lua.
...
hit-testing, `sys.save`/`sys.load` persistence, and Lua closure patterns.
...
- 4×4 grid rendered as colored GUI box nodes.
...
- Combo multiplier: 1 pt base + 1 pt per 3 consecutive hits, capped at 4×.
- Persistent best score via `sys.save` / `sys.load` (JSON-serialised table).
